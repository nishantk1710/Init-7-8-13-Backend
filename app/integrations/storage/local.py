...
a stub: the seed script reads the SAP extracts through it, and it is held to
the same conformance suite the cloud adapter will be. Only reachable when
``ALLOW_NON_AZURE_SQL=1`` is set (see ``app.core.storage.build_storage``) --
production always resolves to Azure Data Lake.
